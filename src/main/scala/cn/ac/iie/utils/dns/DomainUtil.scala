...
object DomainUtil {
  ...
  val majorFanDoaminRegex = "[^.]*(\\.(cn|com|edu|gov|net|org))+$".r
  val minorFanDoaminRegex = "[^.]*(\\.(alibaba|alipay|baidu|icbc|so|unicom))+$".r
  ...
  def getMinorFanDomain(domain:String,regex:Regex=minorFanDoaminRegex)={
    val matches = regex.findFirstIn(domain)
    val result = matches match {
      case Some(s) =>
        val sp = domain.split(s)
  ...
  def getMajorFanDomain(domain:String,majorRegex:Regex=majorFanDoaminRegex,minorRegex:Regex=minorFanDoaminRegex)={
    val matches = majorRegex.findFirstIn(domain)
    val result = matches match {
      case Some(s) =>
        val sp = domain.split(s)
        if(sp.length==0){
      ...
      case None =>getMinorFanDomain(domain,minorRegex)
    }
    result
  }
  ...
  def main(args: Array[String]): Unit = {
    val (domain,level) = getMajorFanDomain("www.vedio.baidu.com.cn")
    val site = JdbcUtil.getWebSite
    val website = JdbcUtil.findWebSiteByDomain(domain,site)
    println(website)
  }
