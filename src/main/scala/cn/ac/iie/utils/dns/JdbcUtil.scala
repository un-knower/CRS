...
object JdbcUtil {
  ...
  def getWebSite: HashMap[String, Website] = {
    ...
    try {
      ...
      while (result.next()) {
        websiteMap.put(result.getString("domain_id"), Website(result.getString("website_id"), result.getString("website_name"), result.getString("website_type"), result.getString("website_url"), result.getString("domain_id")))
      }
    } catch {
  ...
  def findWebSiteByDomain(main_domain: String, webSite: HashMap[String, Website]): Website = {
    var index: Int = -1
    var info = Website("-1", "-1", "-1", "-1", "-1")
    if (main_domain != "-1") {
      val webInfo = webSite
  ...
  def loadTop10000() ={
    var result = ArrayBuffer[mutable.HashMap[String,String]]()
    var conn:Connection = null
    try{
      conn = getConnection()
      val sql = "select domain,`websiteId` ,`websiteName` ,`allNum`  from dm_hot_domain_ascription_0530 where province = '000000' and uu_type = 'all'"
      val statement = conn.prepareStatement(sql)
      val resultSet = statement.executeQuery()
      while (resultSet.next()){
        result.append(
          mutable.HashMap(
            ...
            "websiteId"->resultSet.getString("websiteId"),
            "websiteName"->resultSet.getString("websiteName"),
            "allNum"->resultSet.getString("allNum")
          )
        )
      }
    }catch {
  ...
  def main(args: Array[String]): Unit = {
    val result = loadTop10000()
    var result2 = ArrayBuffer[mutable.HashMap[String,String]]()
    val webSite = JdbcUtil.getWebSite
    for(map <- result){
      val domain = map("domain")
      val (fandomain,level) = DomainUtil.getMajorFanDomain(domain)
      val websitename = JdbcUtil.findWebSiteByDomain(fandomain,webSite)
      result2.append(mutable.HashMap(
        "domain"->domain,
        "fandomain"->fandomain,
        "website_new"->websitename.website_name,
        "website_old"->map("websiteName"),
        "count"->map("allNum")
      ))
    }
    var res = 0
    ...
    try{
      conn = getConnection()
      var sql = "INSERT INTO result (domain,fandomain,website_old,website_new,count) VALUES"

      val it = result2.iterator
      while(it.hasNext){
        val map = it.next()
        var part = s"('${map("domain")}','${map("fandomain")}','${map("website_old")}','${map("website_new")}','${map("count")}'),"
        sql+=part
      }
      sql = sql.substring(0,sql.length-1)
      val statement = conn.prepareStatement(sql)
